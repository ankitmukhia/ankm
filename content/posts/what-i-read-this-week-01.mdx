...
In inference, we generate new data from the model by observing the patterns it has learned in its network parameters. The network gives us a probability vector, which tells us how likely each possible next token is.
Instead of picking randomly, we can think of this as flipping a biased coin—where the outcome isn’t a simple 50/50 chance like a fair coin but is instead weighted based on the probability vector. The higher the probability of a word, the more likely it is to be chosen next.
...
<Sticker>
 ...
 1. **Concept Explanation** – Background knowledge is provided. 
 2. **Worked Examples** – Solved problems show step-by-step solutions. Experts explaining the solution.
 3. **Practice Problems** – Questions are given with only final answers; you figure out the solution yourself.
